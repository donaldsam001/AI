...
def blind_search_dfs(start, goal):
    stack = [[start]]
    visited = set()

    while stack:
        path = stack.pop() # get the last element (LIFO)
        node = path[-1]
        ...
        if node not in visited:
            visited.add(node)
            # add the contiguous node to stack
...
if __name__ == "__main__":
    result = blind_search_dfs('HN', 'V')
    print("Đường đi DFS:", " -> ".join(result) if result else "Không tìm thấy")